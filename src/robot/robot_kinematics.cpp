...
robot_kinematics_t::robot_constants_t::robot_constants_t() {
}

Matrix62d robot_kinematics_t::robot_constants_t::getArmLimits(int side) const {
	if (side == SIDE_RIGHT) {
		return arm_limits;
	} else {
		return robot_kinematics_t::mirrorLimits(arm_limits, arm_mirror);
	}
}

//FIXME:
//Matrix62d robot_kinematics_t::robot_constants_t::getLegLimits(int side) const {
//	if (side == SIDE_RIGHT) {
//		return leg_limits;
//	} else {
//		return robot_kinematics_t::mirrorLimits(leg_limits, leg_mirror);
//	}
//}

Vector6d robot_kinematics_t::robot_constants_t::getArmOffset(int side) const {
	if (side == SIDE_RIGHT) {
		return arm_offset;
	} else {
		return robot_kinematics_t::mirrorAngles(arm_offset, arm_mirror);
	}
}

//FIXME:
//Vector6d robot_kinematics_t::robot_constants_t::getLegOffset(int side) const {
//	if (side == SIDE_RIGHT) {
//		return leg_offset;
//	} else {
//		return robot_kinematics_t::mirrorAngles(leg_offset, leg_mirror);
//	}
//}

Matrix62d robot_kinematics_t::mirrorLimits(const Matrix62d& orig, const IntArray& mirror) {
...
Vector6d robot_kinematics_t::mirrorAngles(const Vector6d& orig, const IntArray& mirror) {
...
void robot_kinematics_t::_DH2HG(Isometry3d &B, double t, double f, double r, double d) {
...
void robot_kinematics_t::_armFK(Isometry3d &B, const Vector6d &q, int side) const {
	Isometry3d hand;
	...
	_armFK(B, q, side, hand);
}

void robot_kinematics_t::_armFK(Isometry3d &B, const Vector6d &q, int side,
	...
	B = neck;
	for (int i = 0; i < 6; i++) {
		_DH2HG(T, t(i)+q(i)-offset(i), f(i), r(i), d(i));
		B = B*T;
	}
	B = B*endEffector;
...
void robot_kinematics_t::_armIK(Vector6d &q, const Isometry3d& B, const Vector6d& qPrev, int side) const {
	// Hand
	Isometry3d hand;
	hand(0,0) =  1; hand(0,1) =  0; hand(0,2) = 0; hand(0,3) =   0;
	hand(1,0) =  0; hand(1,1) =  0; hand(1,2) =-1; hand(1,3) =   0;
	hand(2,0) =  0; hand(2,1) =  1; hand(2,2) = 0; hand(2,3) =   0;
	hand(3,0) =  0; hand(3,1) =  0; hand(3,2) = 0; hand(3,3) =   1;
	_armIK(q, B, qPrev, side, hand);
}

void robot_kinematics_t::_armIK(Vector6d &q, const Isometry3d& B, const Vector6d& qPrev, int side,
                    const Isometry3d &endEffector) const {
